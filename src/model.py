...
class CNN(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(1024, 2048, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(2048)
        self.conv2 = nn.Conv2d(2048, 2048, kernel_size=3, padding=1, groups=32, bias=False)
        self.bn2 = nn.BatchNorm2d(2048)
        self.conv3 = nn.Conv2d(2048, 2048, kernel_size=1, bias=False)
...
class Model(nn.Module):
    def __init__(self, vit, args, num_labels=2):
        super().__init__()
        self.vit = vit
        self.cnn = CNN()
        ...
        if focal_alpha is not None and focal_alpha >= 0:
            alpha = focal_alpha
    ...
    def forward(self, pixel_values, labels=None):
        ...
        batch_size, width_height, channels = hidden_state.shape
        width_height = int(math.sqrt(width_height))
        hidden_state = hidden_state.view(batch_size, width_height, width_height, channels)
        ...
        hidden_state = self.dropout(self.fc(hidden_state))
